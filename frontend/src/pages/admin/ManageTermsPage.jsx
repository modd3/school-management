...
import {
  ...
  FaBookOpen,
  FaFileAlt,
  FaCalendarAlt, // For Manage Terms
...
const DashboardPage = () => {
  ...
  const renderRoleActions = (role) => {
    switch (role) {
      case 'admin':
        return (
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Link to="/admin/reports/generate" className="dashboard-link"> {/* NEW ADMIN LINK */}
      ...
      case 'class_teacher':
      case 'subject_teacher': // Grouped as they share some links
        return (
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            <Link to="/teacher/my-subjects" className="dashboard-link"> {/* NEW TEACHER LINK */}
              <FaBookOpen className="inline-block mr-2" />
            ...
            <Link to="/teacher/my-results" className="dashboard-link"> {/* Renamed for clarity */}
              <FaListAlt className="inline-block mr-2" />
              My Students' Results
            </Link>
            {/* Additional teacher links can go here */}
            {role === 'class_teacher' && (
              <>
      ...
      case 'student':
        return (
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            <Link to="/student/my-subjects" className="dashboard-link"> {/* NEW STUDENT LINK */}
              <FaBookOpen className="inline-block mr-2" />
  ...
  const handleLogout = async () => {
    // The logout function from useAuth() should handle both API call and local state/storage clearing
    await logout();
  };
