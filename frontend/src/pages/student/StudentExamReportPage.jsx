...
import { getStudentTermReport } from '../../api/results';
...
export default function StudentExamReportPage() {
  ...
  useEffect(() => {
    ...
    const fetchReport = async () => {
      ...
      setError(null);
      try {
        const data = await getStudentTermReport(selectedYear, selectedTerm);
        setReport(data);
      } catch (err) {
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Term Report</h1>
      ...
      {report && (
        <div>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div><strong>Total Marks:</strong> {report.summary.totalMarks}</div>
