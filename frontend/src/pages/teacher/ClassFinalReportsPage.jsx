        ...
        <table className="min-w-full divide-y divide-gray-200 print:divide-black">
          ...
          <tbody className="bg-white divide-y divide-gray-200 print:divide-black">
            {allSubjects.map((subjectId, index) => {
              ...
              const subjectResults = reports.reports
                .map(report => report.finalResults.find(s => s.subject === subjectId))
                .filter(Boolean);
              
              const studentsCount = subjectResults.length;
              const totalPercentage = subjectResults.reduce((sum, result) => sum + Number(result.finalPercentage), 0);
      ...
      <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200 print:bg-white print:border-gray-300">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <span className="font-semibold ml-2 print:text-black">{reports.reports.length}</span>
          </div>
          <div>
            <span className="text-gray-600 print:text-black">Class Average:</span>
            <span className="font-semibold ml-2 print:text-black">
              {reports.reports.length > 0 ? 
                (reports.reports.reduce((sum, report) => sum + Number(report.averageMarks), 0) / reports.reports.length).toFixed(1) + '%' : 
                '--'
              }
            </span>
          </div>
          ...
          <div>
            <span className="text-gray-600 print:text-black">Term:</span>
            <span className="font-semibold ml-2 print:text-black">{reports.term}</span>
          </div>
        </div>
      </div>
    </div>
